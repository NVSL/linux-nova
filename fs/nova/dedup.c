...
// Return the number of new write entries to append
// Return the number of duplicate data pages
int nova_dedup_num_new_write_entry(short *target, int num_pages){
  int i;
  int ret=0;
  for(i=0;i<num_pages;i++){
    if(target[i] == 1){ // duplicate
      ret++;
    }   
  }
  return ret;
...
int nova_dedup_FACT_init(struct super_block *sb){
  ...
  char fill[NOVA_DEF_BLOCK_SIZE_4K];
  memset(fill,0,NOVA_DEF_BLOCK_SIZE_4K);
  for(i =start; i<=end;i++){
    target_index = NOVA_DEF_BLOCK_SIZE_4K * (FACT_TABLE_START + i);
    addr = (void*)nova_get_block(sb,target_index);

    nova_memunlock_range(sb,addr, NOVA_DEF_BLOCK_SIZE_4K, &irq_flags);
    memcpy_to_pmem_nocache(addr, &fill, NOVA_DEF_BLOCK_SIZE_4K);  
    nova_memlock_range(sb,addr, NOVA_DEF_BLOCK_SIZE_4K, &irq_flags);
  }
...
int nova_dedup_FACT_insert(struct super_block *sb, struct fingerprint_lookup_data* lookup){
  ...
  */
  ...
  if(ret){ // duplicate data page detected
    ...
    printk("Duplicate Page detected,index: %lld,  ref_count, Up_Count is %lld  %lld\n",index,te.count>>32, te.count);
  }
  ...
  memcpy_to_pmem_nocache(pmem_te, &te, NOVA_FACT_ENTRY_SIZE - 20); // don't write delete, lock, pdding
...
int nova_dedup_entry_update(struct super_block *sb, struct nova_inode_info_header *sih, u64 begin_tail){
  ...
  while(curr_p && curr_p != sih->log_tail){
    ...
    PERSISTENT_BARRIER();
    entry->dedup_flag=0;  // Dedup finish
    nova_flush_buffer(&entry->dedup_flag,CACHELINE_SIZE,1);
...
int nova_dedup_test(struct file * filp){
  ...
  do{
	...
	entry_data.dedup_flag = 2; // flag is set to 2 - in process
